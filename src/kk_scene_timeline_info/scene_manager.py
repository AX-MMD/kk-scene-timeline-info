...
class SceneTimelineInfoManager:
    def __init__(self, *, config: "Config"):
        self.config = config
        self.skip_already_tagged: typing.Optional[bool] = None
        self.file_info_pattern = r"^(?:\[(?P<author>[^\]]+)\]) (?P<filename>.+?) (?:\((?P<duration>\d+m\d{1,2})\))?.*$"

    def _check_skip_status(self) -> bool:
        if self.skip_already_tagged is None:
            i = (
                input("Skip files that already have timeline info? (y/n): ")
                .strip()
                .lower()
            )
            while i not in ("y", "n", "yes", "no"):
                i = input("Invalid input. Please enter 'y' or 'n': ").strip().lower()
            self.skip_already_tagged = i in ("y", "yes")

        return self.skip_already_tagged

    def _extract_info_from_duration_str(
        self, duration_str: typing.Optional[str]
    ) -> typing.Tuple[typing.Optional[str], float]:
        if duration_str is None:
            img_type = None
            duration = 0.0
        elif duration_str == "static":
            img_type = "static"
            duration = 0.0
        elif duration_str == "dynamic":
            img_type = "dynamic"
            duration = 0.0
        elif duration_str.startswith("dynamic;"):
            img_type = "dynamic"
            m = re.match(r"dynamic;(\d+)m(\d{2})", duration_str)
            if m:
            ...
            m = re.match(r"(\d+)m(\d{1,2})", duration_str)
            if m:
                minutes = int(m.group(1))
                seconds = int(m.group(2))
                duration = minutes * 60 + seconds
    ...
    def _extract_name_info(
        ...
        duration_str = match.group("duration") if match else None
        ...
        return filename, author, duration_str, set(tags)
    ...
    def _rename_folder(self, folder_path: str, folder_name: str, duration_str: str):
        ...
        print(f"Done:: '{name}' -> '{new_folder_name}'")
        return new_folder_name

    def _rename_file(
        self, folder_path: str, author_name: str, filename: str
        ...
        img_type, duration = self._extract_info_from_duration_str(duration_str)

        if img_type is not None and self._check_skip_status():
            print(f"Skip Done:: '{filename}'")
            return filename, img_type, duration

        # Get info from the file data
        ...
        if not self.config.display_only:
            try:
                ...
                while u_input not in (
                    ...
                    u_input = (
                        input(
                            "Invalid choice. Please enter copy/skip/replace/quit or their shortcut: "
                        )
                        .strip()
                        .lower()
                    )
